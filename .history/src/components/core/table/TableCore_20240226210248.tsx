...
const TableCore = <T extends object>({ columns, data, paging }: TableCoreProps<T>) => {
  ...
  const rowPerPageOption = Array.from({ length: Math.floor(data.length / (paging?.perPage || 5)) }, (_, i) =>
    (i + 1) * (paging?.perPage || 5) >= data.length ? { label: 'All', value: -1 } : (i + 1) * (paging?.perPage || 5),
  );
  return (
