...
const TableCore = <T extends object>({ columns, data, paging }: TableCoreProps<T>) => {
  const rowsPerPageOptions = [5, 10, 25, { label: 'All', value: -1 }].filter((value) => typeof value === 'number' && value < data.length);

  const [rowsPerPageOptions, setRowsPerPageOptions] = React.useState(rowsPerPageOptions);
  return (
