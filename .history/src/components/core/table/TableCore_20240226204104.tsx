...
const TableCore = <T extends object>({ columns, data, paging }: TableCoreProps<T>) => {
  ...
  const subData = paging ? data.slice((paging?.page || 0) * perPage, ((paging?.page || 0) + 1) * perPage) : data;

  // data.length = 30 => rowsPerPageOptions [5, 10, 25, { label: 'All', value: -1 }]
  // data.length = 10 => rowsPerPageOptions [5, { label: 'All', value: -1 }]
  // data.length = 5 => rowsPerPageOptions [{ label: 'All', value: -1 }]
