  ...
  return (
    <Breadcrumbs
      ...
      >
        {path.length <= 3 ? (
          path.map((p, index) => (
            <Link key={index} underline="hover" color="inherit" href={`/user/share/${p.id}`} className="text-blue-600 hover:underline">
              {p.name}
            </Link>
          ))
        ) : (
          <>
            ...
            <Link underline="hover" color="inherit" href={`/user/share/${path[path.length - 2].id}`} className="text-blue-600 hover:underline">
