...
const ShareFolder = () => {
  ...
  <DriveLayout
    ...
    bodyLeft={
      viewMode === 'grid' ? (
        <DriveGridView entries={data} parent="shared" isLoading={isLoading} curDir={parents[parents.length - 1]} isScrolling={isScrolling} />
      ) : (
        <DriveListView entries={data} parent="shared" curDir={parents[parents.length - 1]} isLoading={isLoading} isScrolling={isScrolling} />
      )
    }
    sidePanel={
      <SidePanel
